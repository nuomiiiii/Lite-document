...
部分主题请求延迟任务清单时没有携带服务器 UUID。服务端会按当前任务关联过滤该节点的历史延迟；主题仍应按节点过滤任务列表，避免把未关联任务显示到当前服务器页面。

## PWA 与站点信息

公开大屏的 Web App Manifest 使用后台设置的站点名称、描述和 `/favicon.ico`。响应使用标准 JSON，并以 `no-store` 避免后台改名后继续使用旧缓存。第三方主题没有提供 manifest 时，服务端会生成合理默认内容。
