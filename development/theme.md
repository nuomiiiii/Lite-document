...
- 压缩后的主题 ZIP 最多 128 MiB。
- 最多 10,000 个文件。
- 单文件解压后最多 128 MiB。
- 全包解压后最多 512 MiB。
- `Lite-theme.json`（或旧兼容清单 `komari-theme.json`）最多 1 MiB。

压缩包限制在直接上传、更新已安装主题、GitHub 安装和主题市场安装中一致。这些是安全上限，不是推荐体积；实际主题应尽量保持在数 MiB 内，并压缩图片和字体。

网络安装只接受 HTTP/HTTPS 地址，并会检查 DNS 解析结果和每次重定向。主题仓库、清单、预览图和 ZIP 必须位于可公开访问的地址；指向本机、私网或保留地址的下载会被拒绝。主题市场或仓库需要内部网络资源时，应改用管理员本地上传 ZIP，不要要求用户关闭下载保护。
...
`common:getNodes` 只返回主题展示所需字段，即使管理员已经登录，也不会返回 Agent Token、Agent 版本、私有备注、部署状态、远程控制状态和节点时间戳。完整字段边界见[兼容与公共接口](/development/compatibility#主题节点资料)。

### 私有站点

私有站点登录前仍可能加载当前主题，以保持访问提示与主题风格一致。此时应先读取公开站点信息和当前登录状态；确认未登录后只显示访问提示，不要创建实时 RPC 连接。

RPC 返回未认证或私有站点拒绝时，应关闭连接并暂停重试，等用户登录或主动刷新后再恢复。持续重连 `/api/rpc2` 只会产生重复 `401`，也会增加服务端和浏览器负担。
