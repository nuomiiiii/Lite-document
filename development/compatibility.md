...
`2.2.1` 起，服务端会把可共享条件的多指标、多节点查询合并为批量扫描。这项优化不改变公共接口的 JSON 字段、层级、数据类型、点顺序、标签、点数限制、空数据填充和可见节点过滤，现有主题不需要为批量引擎单独适配。

`2.2.2` 起，延迟记录、指标曲线和统计接口按当前任务关联过滤。取消服务器与任务的关联后，该节点不再返回该任务的历史延迟；重新关联后仍可查看保留的数据。主题请求任务清单时应携带服务器 UUID。

仪表盘跳转属于另一项显式能力。主题可以在 `komari-theme.json` 中声明服务器详情、网络总览和 Ping 任务参数；未声明时使用兼容回退。具体格式见 [主题开发指南](/development/theme#仪表盘导航)。

从上游迁入时，`1.3.1` 的 `metric_labels` 与 `1.4.x` 的 `metric_label_sets` 都会进入同一套 V4 迁移。公共查询接口不暴露这些内部表名。如果同一份数据库同时包含两套完整上游布局，迁移会拒绝继续。
