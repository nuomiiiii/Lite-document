...
请使用[版本与功能范围](/guide/versioning)中列出的配套 Lite-agent。Lite-agent `2.3.1.1` 仅使用协议 2，并提供远程控制正向开关、远程执行去重、Lite 安装目录迁移和断线重连。只支持旧协议的 Agent 无法继续接入，应先升级。
...
Lite-agent `2.3.1.1` 会在本地记录最近 24 小时、最多 256 条远程执行任务状态。通过 WebSocket 直接下发和断线后的排队投递使用同一套检查，同一个 `task_id` 不会重复执行。
