...
可为面板域名的 `/api/clients` 建立单独的 Bypass 规则。Agent 的其他接口不需要因此全部公开；如果 Agent 也经过 Cloudflare Access，应使用 Service Auth，并在 Agent 中同时配置对应的 Client ID 和 Client Secret。放通 `/api/clients` 不等于取消 Lite 权限校验，管理员登录、2FA、会话和节点权限仍由 Lite 验证。
