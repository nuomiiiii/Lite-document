...
自 Lite `2.3.2`、Lite-agent `2.3.1.1` 起，自动发现注册接口和后台入口已经下线，Agent 不再通过自动发现创建新节点；当前 Lite `2.3.3` 与 Lite-agent `2.3.3.0` 继续沿用这一规则。新安装、批量部署和 Docker 重建都应先在 Lite 后台添加节点，再使用该节点“部署指令”中生成的普通命令。
